...
export default function BoardPage() {
  ...
  // Touch UX: default to View mode on touch devices to allow two-finger / native scrolling
  ...
  // Touch roster placement: tap a roster player (arms), then tap the board to place
  ...
  function commitInlineEdits() {
    // Forces HtmlBoard to persist any active inline text/note edits before save/print/reload
  ...
  return (
    <main className="sb-print-root h-screen overflow-hidden">
      {/* Top bar */}
      <div className="sb-no-print flex items-center justify-between px-6 py-4 border-b bg-white relative z-40">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <button
            ...
            title={editMode ? "Edit mode: move/add items" : "View mode: scroll without moving items"}
          >
            {editMode ? "Edit" : "View"}
          </button>
        ...
        <div className="flex items-center gap-3 sb-no-print">
          <Link className="underline" href="/app/teams">
            Teams
          </Link>
          {board?.team_id ? (
            <Link className="underline" href={`/app/teams/${board.team_id}`}>
              Boards
            </Link>
          ) : null}
        ...
        <div className="flex h-[calc(100vh-73px)]">
          ...
          {!sidebarCollapsed ? (
            <aside className="w-96 shrink-0 border-r p-4 overflow-auto bg-gray-50 relative z-30">
              ...
              {isTouch && editMode && armedPlayer ? (
                <div className="text-xs text-blue-700 bg-blue-50 border border-blue-200 rounded p-2 mb-2">
                  Tap on the board to place <span className="font-semibold">{armedPlayer.name}</span>.
                  <button
                    type="button"
                    className="ml-2 underline"
                    onClick={() => setArmedPlayer(null)}
                  >
                    cancel
                  </button>
                </div>
                ...
                <div className="space-y-2">
                  {filteredPlayers.map((p, idx) => (
                    <div
                      ...
                      className={`border rounded bg-white ${editMode && !isTouch ? "cursor-grab active:cursor-grabbing" : "cursor-pointer"} ${armedPlayer?.id === p.id ? "ring-2 ring-blue-500/60" : ""}`}
                      draggable={editMode && !isTouch}
                      onDragStart={(e) => {
                        if (!editMode || isTouch) return;
                        onPlayerDragStart(e, p);
                      }}
                      onClick={() => {
                        if (!isTouch) return;
                        // Touch workflow: tap roster to arm, then tap board to place
            ...
            <aside className="sb-no-print w-12 shrink-0 border-r bg-gray-50 relative z-30 flex flex-col items-center py-3">
              <button
                type="button"
          ...
          {/* Board */}
          <section className="sb-print-canvas flex-1 relative z-0 overflow-hidden">
            <HtmlBoard
              editMode={true}
