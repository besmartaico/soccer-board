...
export default function TeamBoardsPage() {
  ...
  async function createBoard() {
    ...
    try {
      ...
      // IMPORTANT: Create via API so editors can create boards even if RLS blocks direct inserts.
      const { data: sess } = await supabase.auth.getSession();
      const token = sess.session?.access_token;
      if (!token) throw new Error("Missing session token.");

      const resp = await fetch(`/api/teams/${teamId}/boards`, {
        method: "POST",
        headers: {
      ...
      const created = json.board as BoardRow;

      setNewBoardName("");
      setBoards((cur) => [created as any, ...cur]);

      // Navigate to the newly created board
      if (created?.id) {
        router.push(`/app/boards/${created.id}`);
      }
    } catch (e: any) {
    ...
    <main className="min-h-screen bg-white">
      <div className="px-8 py-6 border-b">
        <div className="min-w-0">
          ...
          <div className="text-gray-600">Boards{memberRole ? ` • Your access: ${memberRole}` : ""}</div>
        </div>
      </div>
        ...
        <div className="p-8 max-w-5xl mx-auto space-y-6">
          ...
          <div className="border rounded-2xl p-6">
            ...
            <div className="border rounded-xl p-4 mt-4">
              ...
              {!canCreateBoards ? (
                <div className="text-sm text-gray-500 mt-2">You currently have viewer access.</div>
              ) : null}
            </div>
          </div>
        </div>
