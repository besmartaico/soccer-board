...
export function HtmlBoard({
  ...
  onToolChange,
  cardSizeMode = "large",
  // ✅ touch-friendly roster placement
  armedPlayer,
  ...
  // ✅ track the active editable element so Save/Print can commit without blur
  const activeEditElRef = useRef<HTMLDivElement | null>(null);

  // Touch pointers for two-finger scroll (Edit mode only)
  ...
  // ---------- commit edits event (Save/Print safety) ----------
  useEffect(() => {
    function commitEdits() {
      const id = editingIdRef.current;
      const el = activeEditElRef.current;

      if (!id || !el) return;

      const nextText = el.innerText ?? "";
      updateObject(id, { text: nextText });
      setEditingId(null);
      activeEditElRef.current = null;
    }

    window.addEventListener("soccerboard:commit-edits", commitEdits);
    return () => window.removeEventListener("soccerboard:commit-edits", commitEdits);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  // ---------- pointer tracking (capture phase) ----------
  function onPointerDownCapture(e: React.PointerEvent) {
    if (e.pointerType !== "touch") return;

    // Track pointers even if the touch starts on a card/object
    pointersRef.current.set(e.pointerId, { x: e.clientX, y: e.clientY, pointerType: e.pointerType });

    // Only run the manual 2-finger panning logic in edit mode.
    // In view mode we rely on native scrolling (touchAction pan-x/pan-y).
    if (!editMode) return;
  ...
  function onPointerDownCanvas(e: React.PointerEvent) {
    // Track touch pointers for two-finger scroll (edit mode only; capture handles most cases)
    if (e.pointerType === "touch") {
      pointersRef.current.set(e.pointerId, { x: e.clientX, y: e.clientY, pointerType: e.pointerType });

      if (editMode && pointersRef.current.size >= 2) {
        const pts = Array.from(pointersRef.current.values()).slice(0, 2);
        twoFingerRef.current = {
          active: true,
          lastCx: (pts[0].x + pts[1].x) / 2,
    ...
    // ✅ Touch tap-to-place: armed roster player
    if (editMode && armedPlayer && e.pointerType === "touch") {
      const pt = clientToBoard(e.clientX, e.clientY);
      ...
      const id = `${armedPlayer.id || armedPlayer.name}-${Date.now()}-${Math.random().toString(16).slice(2)}`;
      const nextX = clamp(pt.x - w / 2, 0, canvasWidth - w);
      const nextY = clamp(pt.y - h / 2, 0, canvasHeight - h);
  ...
  function onPointerMove(e: React.PointerEvent) {
    // ✅ Manual two-finger pan ONLY in edit mode (in view mode use native scrolling)
    const tf = twoFingerRef.current;
    if (editMode && tf?.active && e.pointerType === "touch") {
    ...
    >
      <div
        ...
        style={{
          ...
          height: canvasHeight,
          ...bgStyle,
          // ✅ View mode: allow native pan/scroll; Edit mode: lock for pointer handling
            ...
            >
              ...
              {isEditing ? (
                <div
                  ...
                  onFocus={(e) => {
                    activeEditElRef.current = e.currentTarget;
                  }}
                  onPointerDown={(e) => {
